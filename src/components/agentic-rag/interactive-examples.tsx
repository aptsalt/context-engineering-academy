...
import { ragCodeExamples } from "@/lib/agentic-rag-data";

export function RagInteractiveExamples() {
  const [activeExample, setActiveExample] = useState(ragCodeExamples[0].id);
  ...
  const current =
    ragCodeExamples.find((e) => e.id === activeExample) ?? ragCodeExamples[0];

  return (
    <div className="space-y-8">
      <div className="prose prose-invert max-w-none">
        <p className="text-lg leading-relaxed text-foreground/90">
          See RAG patterns in action. Each example shows a{" "}
          <strong className="text-red-600">naive approach</strong> and its{" "}
          <strong className="text-green-600">production-grade fix</strong>.
        ...
        <CardContent className="space-y-4">
          ...
          {!showGood[current.id] ? (
            <div className="space-y-3">
              <CodeBlock code={current.bad.code} label={current.bad.label} />
            ...
            <div className="space-y-3">
              <CodeBlock code={current.good.code} label={current.good.label} />
