...
import { multiAgentAntiPatterns } from "@/lib/multi-agent-data";
...
export function MultiAgentAntiPatterns() {
  return (
    <div className="space-y-8">
      <div className="prose prose-invert max-w-none">
        <p className="text-lg leading-relaxed text-foreground/90">
          Multi-agent systems introduce unique failure modes that don&apos;t
          exist in single-agent architectures. These anti-patterns are
          documented from real production failures across the industry —
          understanding them is the difference between a demo that works and a
          system that stays working.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {multiAgentAntiPatterns.map((pattern) => (
          <Card
            key={pattern.name}
