...
import { multiAgentCodeExamples } from "@/lib/multi-agent-data";

export function MultiAgentInteractiveExamples() {
  const [activeExample, setActiveExample] = useState(multiAgentCodeExamples[0].id);
  ...
  const current = multiAgentCodeExamples.find((e) => e.id === activeExample) ?? multiAgentCodeExamples[0];

  return (
    <div className="space-y-8">
      <div className="prose prose-invert max-w-none">
        <p className="text-lg leading-relaxed text-foreground/90">
          See multi-agent patterns in action. Each example shows a{" "}
          <strong className="text-red-600">common mistake</strong> and its{" "}
          <strong className="text-green-600">production-ready fix</strong>.
      ...
      <div className="flex flex-wrap gap-2">
        {multiAgentCodeExamples.map((example) => (
          <Button
            key={example.id}
